...
void kmap(void* vaddr, void* paddr) {
    ...
    if (L1_table[L1_index].valid == 0) {
        ...
        void* new_table = init_kernel_table();
        L1_table[L1_index].next_lvl_table = ((unsigned long int) new_table) >> 12;
    }
    ...
    if (L2_table[L2_index].valid == 0) {
        ...
        void* new_table = init_kernel_table();
        L2_table[L2_index].next_lvl_table = ((unsigned long int) new_table) >> 12;
    }
...
static void* init_kernel_table() {
    ...
    set_recursive_entry(mount, table);
